...
public final class RedisUtil {
    ...
    private static String ADDR = "127.0.0.1";//redis服务器
...
//    如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted（耗尽）
    ...
    //等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接报出JedisConnectionException;
    private static int MAX_WAITMILLIS= 10000;

    private static int TIMEOUT = 10000;

    //在borrow一个jedis实例时，是否提示进行validate操作；如果为true，则得到的jedis实例均是可用的；
